...
# Run the application
dotnet run --project src/Minimalhr_ms.Web

# Or run with Aspire (if using)
dotnet run --project src/Minimalhr_ms.AspireHost
```
...
```powershell
dotnet ef database update -c AppDbContext -p src/Minimalhr_ms.Web -s src/Minimalhr_ms.Web
dotnet run --project src/Minimalhr_ms.Web
```
