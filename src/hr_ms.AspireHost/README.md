...
1. Set `hr_ms.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../hr_ms.Infrastructure/hr_ms.Infrastructure.csproj -s hr_ms.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../hr_ms.Infrastructure/hr_ms.Infrastructure.csproj -s hr_ms.Web.csproj
```
